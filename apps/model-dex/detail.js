...
;(function () {
  ...
  const CAPS = [
    { key: 'reasoning', label: '推理', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 1 1 7.072 0l-.548.547A3.374 3.374 0 0 0 14 18.469V19a2 2 0 1 1-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>' },
    { key: 'tool_call', label: '工具调用', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>' },
    { key: 'structured_output', label: '结构化输出', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6M16 13H8M16 17H8M10 9H8"/></svg>' },
    { key: 'attachment', label: '文件附件', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>' },
    { key: 'temperature', label: '温度控制', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 14.76V3.5a2.5 2.5 0 0 0-5 0v11.26a4.5 4.5 0 1 0 5 0z"/></svg>' },
    { key: 'open_weights', label: '开放权重', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>' },
  ];
  ...
  function renderPricing(providers) {
    ...
    // Find cheapest
    let minInput = Infinity, minOutput = Infinity;
    let freeCount = 0;
    providers.forEach(p => {
      if (!p.cost) return;
      if (p.cost.input === 0 && p.cost.output === 0) { freeCount++; return; }
      if (p.cost.input != null && p.cost.input < minInput) minInput = p.cost.input;
      if (p.cost.output != null && p.cost.output < minOutput) minOutput = p.cost.output;
    });

    const inputStr = minInput === Infinity ? '-' : formatPrice(minInput);
    const outputStr = minOutput === Infinity ? '-' : formatPrice(minOutput);
    const sub = freeCount > 0 ? `${freeCount} 个供应商提供免费使用` : `共 ${providers.length} 个供应商`;
  ...
  function renderProviderTable(providers) {
    ...
    const rows = providers.map(p => {
      const cost = p.cost || {};
      const inputPrice = cost.input != null ? formatPrice(cost.input) + '/M' : '-';
      const outputPrice = cost.output != null ? formatPrice(cost.output) + '/M' : '-';
  ...
  function renderBenchmarks(benchmarks) {
    ...
    const items = benchmarks.map(b => {
      const version = b.version ? `<span class="benchmark-version">v${b.version}</span>` : '';
      const metric = b.metric ? `<span class="benchmark-metric">${b.metric}</span>` : '';
      return `
