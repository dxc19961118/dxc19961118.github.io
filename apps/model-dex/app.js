...
;(function () {
  ...
  function processData(catalog) {
    ...
    for (const model of models) {
      ...
      const official = providers.find(p => p.provider.id === model.lab) || providers[0];
  ...
  function renderModelCard(model) {
    ...
    const tags = [];
    if (model.reasoning) tags.push('<span class="tag tag-reasoning"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 1 1 7.072 0l-.548.547A3.374 3.374 0 0 0 14 18.469V19a2 2 0 1 1-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>推理</span>');
    if (model.tool_call) tags.push('<span class="tag tag-tool"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>工具</span>');
    if (model.structured_output) tags.push('<span class="tag tag-structured"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/></svg>结构化</span>');
    if (model.open_weights) tags.push('<span class="tag tag-open"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg>开源</span>');
  ...
  function renderDetail(modelId) {
    ...
    if (!model) {
      dom.viewDetail.innerHTML = '<p style="text-align:center;padding:40px;color:var(--fg-muted)">模型未找到</p>';
      return;
    }
