...
(function () {
  ...
  const LOCAL_SNAPSHOT_URL = "./data/catalog.json";
  const LOGO_BASE = "https://models.dev/logos/labs";
  const REMOTE_TIMEOUT_MS = 5000; // 远程 API 超时 5 秒
  ...
  const state = {
    ...
    currentModelId: null,
    dataSource: null, // 'remote' | 'snapshot' — 数据来源
    filters: {
      lab: "",
  ...
  function bindEvents() {
    ...
    // 降级横幅：重试在线数据
    $("#btnRetryOnline").addEventListener("click", () => {
      hideFallbackBanner();
      dom.loading.classList.remove("hidden");
      loadData();
    });
  ...
  async function loadLocalSnapshot() {
    ...
    if (!res.ok) throw new Error(`本地快照加载失败: HTTP ${res.status}`);
    return res.json();
  }
  ...
  async function loadData() {
    ...
    try {
      ...
      console.log("[model-dex] 数据来源：远程 API (models.dev)");
    } catch (err) {
      console.warn("[model-dex] 远程 API 不可达，尝试降级到本地快照:", err.message);

      // 2. 降级到本地快照
      try {
        catalog = await loadLocalSnapshot();
        state.dataSource = "snapshot";
        console.log("[model-dex] 数据来源：本地快照 (离线数据)");
        showFallbackBanner();
      } catch (snapshotErr) {
        // 3. 本地快照也失败，显示错误
        console.error("[model-dex] 降级失败:", snapshotErr);
        dom.loading.classList.add("hidden");
        dom.error.classList.remove("hidden");
        dom.errorMsg.textContent =
          `在线数据不可达，本地快照也加载失败：${snapshotErr.message}`;
        return;
      }
    }
